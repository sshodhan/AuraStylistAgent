  ...
  if (!outfit) {
    return (
      <div className="h-[70dvh] flex flex-col items-center justify-center p-12 text-center space-y-6">
        ...
        <button onClick={() => onTabChange(AppTab.STYLIST)} className="text-[10px] font-black text-indigo-600 uppercase tracking-widest bg-indigo-50 px-6 py-3 rounded-xl">Go to Stylist</button>
      </div>
    );
  }
    ...
    <div className="space-y-4 pb-12 animate-in fade-in duration-500">
        ...
        <div className="flex items-center justify-between cursor-pointer" onClick={() => setIsHubExpanded(!isHubExpanded)}>
          <div className="flex items-center gap-2">
            ...
            {!isHubExpanded && userPhoto && (
              <motion.div initial={{ opacity: 0, scale: 0.5 }} animate={{ opacity: 1, scale: 1 }} className="w-5 h-5 rounded-full overflow-hidden border border-white/40 ml-1">
          ...
          {isHubExpanded && (
            <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.4, ease: "circOut" }} className="overflow-hidden">
              <div className="space-y-4 pt-4 relative z-10 text-white">
                <div className="absolute top-0 right-0 p-4 opacity-10 -z-10"><Sparkles className="w-24 h-24 text-white" /></div>
                <h2 className="text-lg font-black tracking-tight leading-tight">Personalize Your Look</h2>
                <p className="text-[10px] opacity-80 font-medium leading-relaxed max-w-[240px]">Analyze your specific features to map the styling verdict onto your likeness.</p>
                <div className="pt-2">
                  {userPhoto ? (
                    <div className="flex items-center gap-3 bg-white/10 backdrop-blur-md p-2 rounded-2xl border border-white/20">
                      <div className="w-12 h-12 rounded-xl overflow-hidden border border-white/40 shrink-0"><img src={userPhoto} alt="Ref" className="w-full h-full object-cover" /></div>
                      <div className="flex-1">
                        <p className="text-[9px] font-black uppercase tracking-widest text-indigo-100">Ready to Render</p>
                        <p className="text-[8px] opacity-60">Personalized Mode Active</p>
                      </div>
                      <button onClick={(e) => { e.stopPropagation(); setUserPhoto(null); }} className="p-1.5 hover:bg-white/10 rounded-lg transition-colors"><X className="w-4 h-4 text-white" /></button>
                    </div>
                  ) : (
                    <button onClick={(e) => { e.stopPropagation(); fileInputRef.current?.click(); }} className="w-full py-4 bg-white/20 hover:bg-white/30 backdrop-blur-md border-2 border-dashed border-white/40 rounded-2xl flex flex-col items-center justify-center gap-2 transition-all active:scale-[0.98]">
      ...
      <div className="bg-white p-5 rounded-[2.5rem] border border-gray-100 shadow-sm space-y-4 mx-1">
        <div className="flex items-center justify-between">
          <h2 className="text-sm font-black flex items-center gap-2 text-gray-900 uppercase tracking-widest"><Sparkles className="text-indigo-600 w-4 h-4" />Style Lab</h2>
          <div className="bg-indigo-50 px-2 py-1 rounded-md"><span className="text-[8px] font-black text-indigo-600 uppercase tracking-widest">Gemini 3 Pro</span></div>
        </div>
        <div className="flex gap-2 p-1 bg-gray-50 rounded-2xl border border-gray-100">
          {["1K", "2K", "4K"].map((s) => (
            <button key={s} onClick={() => setSize(s as any)} className={`flex-1 py-2.5 rounded-xl text-[10px] font-black transition-all ${size === s ? 'bg-white text-indigo-600 shadow-sm' : 'text-gray-400'}`}>{s}</button>
          ))}
        </div>
        <button onClick={handleGenerate} disabled={loading} className={`w-full py-4 rounded-2xl font-black text-[9px] uppercase tracking-[0.2em] transition-all flex justify-center items-center gap-3 active:scale-[0.98] shadow-xl ${userPhoto ? 'bg-gradient-to-r from-indigo-600 to-violet-700 text-white shadow-indigo-100' : 'bg-gray-900 text-white shadow-gray-200'}`}>
          {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : <>{userPhoto && <Zap className="w-4 h-4" />}{userPhoto ? "Personalized Fit Check" : "Generate Lookbook Range"}</>}
        </button>
      </div>
      ...
      <div className="space-y-8">
        {loading ? (
          <div className="mx-1 aspect-[3/4] bg-gray-50 rounded-[3rem] border border-gray-100 flex flex-col items-center justify-center space-y-6">
            <div className="relative"><div className="w-16 h-16 border-4 border-indigo-600 border-t-transparent rounded-full animate-spin"></div><Camera className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-indigo-600 w-6 h-6" /></div>
            <div className="text-center space-y-1 px-12">
              <p className="text-[10px] font-black text-gray-900 uppercase tracking-widest animate-pulse">{userPhoto ? "Mapping to Persona" : "Synthesizing Varieties"}</p>
              <p className="text-[8px] text-gray-400 font-bold uppercase tracking-tighter">Please wait while the AI generates high-fidelity fashion renders...</p>
            </div>
          </div>
        ) : imageUrls ? (
          <div className="space-y-12 px-1">
            {imageUrls.map((url, idx) => (
              <div key={idx} className="space-y-4">
                <div className="relative aspect-[3/4] bg-gray-50 rounded-[3.5rem] overflow-hidden border border-gray-100 shadow-2xl animate-in zoom-in-95 duration-500">
                  <img src={url} alt={`Option ${idx + 1}`} className="w-full h-full object-cover" />
                  
                  {/* Detailed Look Overlay from Screenshot */}
                  <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/90 via-black/40 to-transparent p-10 flex justify-between items-end">
                    <div className="text-white space-y-1">
                      <p className="text-[8px] font-black uppercase tracking-[0.2em] opacity-80">Look Details</p>
                      <h4 className="text-sm font-bold leading-snug max-w-[200px] line-clamp-3">{outfit.outerwear} over {outfit.baseLayer} in {weather.location}</h4>
                      <div className="flex items-center gap-2 pt-1">
                         <div className="w-1.5 h-1.5 rounded-full bg-indigo-400 animate-pulse" />
                         <span className="text-[8px] font-black uppercase tracking-widest text-indigo-200">
                           {userPhoto ? "Personalized Likeness" : `${variationsLabel[idx]} Style`}
                         </span>
                      </div>
                    </div>
                    <div className="flex gap-3">
                       <button onClick={() => setEditingIndex(editingIndex === idx ? null : idx)} className={`p-4 rounded-2xl shadow-xl transition-all ${editingIndex === idx ? 'bg-indigo-600 text-white' : 'bg-white/10 backdrop-blur-md text-white border border-white/20'}`}>
                        <Wand2 className="w-5 h-5" />
                      </button>
                      <a href={url} download={`aura-look-${idx+1}.png`} className="p-4 bg-white rounded-2xl text-indigo-600 shadow-xl active:scale-95 transition-all">
                  ...
                  {editingIndex === idx && (
                    <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="bg-indigo-50 border border-indigo-100 p-6 rounded-[2.5rem] space-y-4 overflow-hidden">
                      ...
                      <div className="relative">
                        <motion.div animate={pulseInput ? { scale: [1, 1.02, 1] } : {}} transition={{ duration: 0.4 }}>
                          <input type="text" ref={inputRef} value={editPrompt} onChange={(e) => setEditPrompt(e.target.value)} placeholder="e.g. 'Add more rain' or 'Make it BW'" className="w-full bg-white border border-indigo-100 rounded-2xl px-5 py-4 text-xs font-medium outline-none pr-16 shadow-sm focus:ring-4 focus:ring-indigo-600/10" />
                        </motion.div>
                        <button onClick={() => handleApplyEdit(idx)} disabled={isEditing || !editPrompt.trim()} className="absolute right-2 top-2 bottom-2 px-4 bg-indigo-600 text-white rounded-xl disabled:bg-gray-300 transition-colors">
                          {isEditing ? <Loader2 className="w-4 h-4 animate-spin" /> : <Send className="w-4 h-4" />}
                        </button>
                      </div>
                      <div className="flex flex-wrap gap-2">
                        {["Night time", "Retro film", "Cyberpunk", "Golden hour"].map(s => (
                          <button key={s} onClick={() => handleSelectSuggestion(s)} className={`text-[9px] font-black uppercase tracking-wider px-4 py-2 rounded-full border transition-all ${editPrompt === s ? 'bg-indigo-600 text-white border-transparent' : 'bg-white text-indigo-600 border-indigo-100 hover:bg-indigo-50'}`}>{s}</button>
                        ))}
                      </div>
                    </motion.div>
                  )}
          ...
          <div className="mx-1 aspect-[3/4] bg-gray-50 rounded-[3rem] flex flex-col items-center justify-center p-12 text-center space-y-4 opacity-40 border border-gray-100">
            ...
            <div className="text-center space-y-2">
              <p className="text-[10px] font-black uppercase tracking-widest text-gray-400">Ready for Session</p>
              <p className="text-[8px] font-bold text-gray-300 uppercase tracking-tighter px-12 leading-relaxed">Trigger generate to see your styling verdict in full high-fidelity renders.</p>
            </div>
          </div>
        )}
      </div>
